...
echo '<h1> <center>Cumpleaños de padres de familia de la semana.</center></h1>';

///Cambiar la ruta!!!!
$cuerpo= "<center><img  src='http://ecolmenares.net/sieweb/altamira/im/logo.jpg'></img></center>";

$cuerpo.='<h1> <center>Cumpleaños de padres de familia de la semana.</center></h1>';
$cuerpo.= "<html> 
	<head> <title>Cumpleaños de la semana.</title> </head> 
	<body>
	<br>
	<p>&nbsp;</p>
";
...
echo '<tr><td>&nbsp;</td></tr><td>&nbsp;</td><tr></tr><tr><td class="a1"><strong>&nbsp;Cumpleaños del día: </strong>'.$fecha->format('d/m/Y').'</td></tr>'; // imprime por ejemplo: 01/01/2014
...
while($row = mysql_fetch_array($cumple_papa))
                    {
					  ...
					  $cuerpo.='<tr><td>&nbsp;'.utf8_encode($row['Nombre']).'&nbsp;</td>
 					  ...
 					  $exportar.=
					  utf8_encode($row['Nombre']).";".$row['fecha_nac_padre'].";".$row['e_mail_padre'].";".$row['telefono_padre'].";".$row['celular_padre'].";".utf8_encode($row['alumno']).";".$row['seccion'].";".$row['grado']."-".$row['grupo'];
					  $exportar.="\n";
...
while($row = mysql_fetch_array($cumple_mapa))
                    {
					  ...
					  ////Cuerpo que se envia al TXT
					  $exportar.=utf8_encode($row['Nombre']).";".$row['fecha_nac_madre'].";".$row['e_mail_madre'].";".$row['telefono_madre'].";".$row['celular_madre'].";".utf8_encode($row['alumno']).";".$row['seccion'].";".$row['grado']."-".$row['grupo'];
					  $exportar.="\n";					  
                    } 
}
    echo '</table>';
...
file_put_contents('CumpleSemana.xls',utf8_decode($cuerpo));
file_put_contents('CumpleSemana.txt',utf8_decode($exportar));

//// Ejecutamos el archivo de PHP para crear el pdf
include_once('pdf_cumple_semana.php');  

//// PHP Mailer
$hostSMTP="smtp.gmail.com";
$smtpPrefix="tls";
$smtpPort=587;
$mailSender="Cumpleaños de la Semana, $n_escuela";
...
$mailPass="U926US<5gG";
		...
		$mail->IsHTML(true);
		$mail->AddAttachment('CumpleSemana.xls', 'CumpleSemana.xls');
		$mail->AddAttachment('CumpleSemana.pdf', 'CumpleSemana.pdf.pdf');
		$mail->Timeout=300;
		...
		$mail->Subject="Cumpleaños de la Semana, $n_escuela";
		$mail->Body=$cuerpo;
...
$dir='CumpleSemana.xls'; //puedes usar dobles comillas si quieres
if(file_exists($dir))
{
if(unlink($dir))
...
$dir='CumpleSemana.pdf'; //puedes usar dobles comillas si quieres
if(file_exists($dir))
{
if(unlink($dir))
...
$dir='CumpleSemana.txt'; //puedes usar dobles comillas si quieres
if(file_exists($dir))
{
if(unlink($dir))
print "<br>El archivo ".$dir." fue borrado";
