...
if($_POST[done]=='S' && strpos($_POST[parentframe],'encuesta.php')!=false){
	mysql_query("update preceptoria_acuerdos set st=2 where alumno=$_POST[alumno] and fec=CURDATE()") or die(mysql_error("No se pudo actualizar el estatus de la entrevista."));
	include('mailerEncuesta.php');
	//include('mailer.php');/////////////////////////////////////////06-10-2012
...
--></script>
 ...
 <body marginheight='0' marginwidth='0' bgcolor='$fondo'>
...
<?
	...
	if($_GET['tabla']=='test_respuestas'){
       ...
       <input type='hidden' id='alumno' name='alumno' value='$_GET[alumno]'>
       <input type='hidden' id='terminado' name='terminado' value='$trmnd'>
	   <input type='hidden' id='parentframe' name='parentframe' value=''>
       <script language='javascript'>
		 document.getElementById('parentframe').value=parent.location.href;
...
function Guardar() {
alert("Encuesta Guardada Satisfactoriamente!")
}
</script>
Terminado: <input type='hidden' id='done' name='done' value='S'>
  ...
  <tr align='left'>
		<th><input type='submit'
			...
			value='Quitar' /><?=$row_ ['nombre']?> Edita:<?=$row_ ['edita']?></th>
				...
				$row = mysql_fetch_array ( $result );
				if ($row_ ['edita'] == 'N')
					$bloqueado .= " disabled='disabled' STYLE='background-color: #FFFFFF;'";
				...
				switch ($campo) {
					case 'seccion' :
						echo "<input size='20' value='" . mysql_result ( mysql_query ( "select nombre from secciones, parametros where seccion='" . $row ["seccion"] . "' and parametros.periodo=secciones.ciclo", $link ), 0, 0 ) . "' STYLE='background-color: #FFFFFF;' readonly>";
						break;
					case 'seccion_ingreso' :
						echo "<input size='20' value='" . mysql_result ( mysql_query ( "select nombre from secciones where seccion='" . $row ["seccion_ingreso"] . "' and ciclo=" . $row ['periodo_ingreso'], $link ), 0, 0 ) . "' STYLE='background-color: #FFFFFF;' readonly>";
						break;
					case 'beca' :
						echo "<input size='40' value='" . mysql_result ( mysql_query ( "select descripcion from becas where beca=" . $row ["beca"], $link ), 0, 0 ) . "' STYLE='background-color: #FFFFFF;' readonly>";
						break;
					case 'beca_2' :
						echo "<input size='40' value='" . mysql_result ( mysql_query ( "select descripcion from becas where beca=" . $row ["beca_2"], $link ), 0, 0 ) . "' STYLE='background-color: #FFFFFF;' readonly>";
						break;
					...
					case 'sexo' :
						...
						if (strtoupper ( $row ["sexo"] ) == "F")
							$sexo = "Femenino";
						echo "<input value='$sexo' size='10' STYLE='background-color: #FFFFFF;' readonly>";
						break;
					case 'activo' :
						...
						echo "<input value='$activo' size=7 STYLE='background-color: #FFFFFF;' readonly>";
						break;
					case '' :
						echo "";
						break;
		...
		if($_GET['tipo']=='alumnos' && $_GET[administra_test]!='S'){
			...
			//S�"LO SI EL ALUMNO YA TIENE HISTORIAL -registros en historial_Alumnos- SE MUESTRA LA TABLA.
